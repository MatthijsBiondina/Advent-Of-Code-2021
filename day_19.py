...
for ii in range(len(data)):
    for jj in range(len(data)):
        ...
        # ONLY ROTATION
        scan2_ = (R[:, None, :] @ scan2[None, :, :, None]).squeeze(-1)

        overl_ = (scan1[None, :, None, :] == scan2_[:, None, :, :])
        overl_ = np.all(overl_, axis=-1)
        overl_ = np.sum(overl_, axis=(-1, -2))

        # ONLY TRANSLATION
        scan2__ = scan2[None, None, :, :] + (scan1[None, :, :] - scan2[:, None, :])[..., None, :]
        # scan2 = (R[:, None, None, None, ...] @ scan2[None, ..., None]).squeeze(axis=-1)

        overl__ = (scan1[None, None, :, None, :] == scan2__[:, :, None, :, :])
        overl__ = np.all(overl__, axis=-1)
        overl__ = np.sum(overl__, axis=(-1, -2))
        ...
        if len(overl) > 0:
            ii = overl[0]
            new_scan2 = scan2[ii[0], ii[1], ii[2]]

        ii = np.argwhere(overl)[0]
